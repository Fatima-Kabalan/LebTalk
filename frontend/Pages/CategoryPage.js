    ...
    return (
      <View style={styles.container}>
        ...
        {category.category_name == "Family" ? (
          ...
          <Image
            source={{ uri: IMAGE_URL + category.category_image }}
            style={styles.img}
          />
        )}
        ...
        <ScrollView>
          ...
          <View style={styles.flatCards}>
            {cards?.map((card, index) => {
              return (
                <FlatCard
                  source={IMAGE_URL + card.card_image}
                  text1={card.english_text}
                  text2={card.arabic_text}
                  voice_note={VOICE_URL + card.voice_note}
                  card_id={card.id}
                />
              );
            })}
          ...
          <View style={styles.testContainer}>
            <ContainedButton
              ...
              onPress={() =>
                navigation.navigate("Questions", { category_id: category.id })
              }
            />
          </View>
        </ScrollView>
